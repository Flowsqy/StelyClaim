...
public class RemoveOwnerSubCommand extends DomainSubCommand {
    ...
    protected void interact(@NotNull CommandContext context, @NotNull OfflinePlayer target) {
        final InteractProtocol protocol = new InteractProtocol(new DomainProtocol(DomainProtocol.Protocol.REMOVE_OWNER, target), getPermChecker());
    ...
    protected void sendMessage(@NotNull CommandContext context) {
        ...
        new DomainMessage().sendMessage(context, usedMessages, "owner", "not", "remove");
    }
